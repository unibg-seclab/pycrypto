...
static void block_init(block_state* self, unsigned char* key, int keylen)
{
    ...
    switch (keylen) {
        ...
        default:
            PyErr_SetString(PyExc_ValueError,
                "AES key must be either 16, 24, or 32 bytes long, so here we need 32, 48 or 64 bytes");
            return;
    }
    ...
    aes_key_setup_enc(self->ek, &(key[keylen]), keylen);
}
